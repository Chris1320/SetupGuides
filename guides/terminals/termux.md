...
3. Install Termux API. `$ apt install termux-api`[^2]
4. Configure access to external storage. `$ termux-setup-storage`[^3]
...
2. Add `sshd -p <port>`[^4] to your `.zshrc` (Or `.bashrc`, if you are not using zsh) file.
...
[^1]: If you encounter an error `W: Some index files failed to download...`, run `$ termux-change-repo` to use a mirror and try again.
[^2]: You might have to use a mirror to download termux-api by using `$ termux-change-repo`.
[^3]: Android might prompt you to allow access to external storage. Press "*Allow*" to continue.
[^4]: The SSH port is usually 22. However, if you are using a non-rooted Android device, use `8022` or any port above 1024.
